...
class Dueling_DQN(nn.Module):
    def __init__(self):
        ...
        self.Linear1_adv = nn.Linear(3136,512)
        self.Linear1_val = nn.Linear(3136,512)

        self.Linear2_adv = nn.Linear(512,action_dim)
    ...
    def forward(self,x):
        ...
        adv = F.relu(self.Linear1_adv(x))
        val = F.relu(self.Linear1_val(x))

        adv = self.Linear2_adv(adv)
        val = self.Linear2_val(val).expand(x.size(0),action_dim)
...
class Agent:
    def __init__(self, mode="training"):
        ...
        self.model_dir=f'Double_Dueling_DQN_{LLM_name}_adjust{adjust_frequency}_sample{sample_rate}_{prompt_type}_temperature{LLM_temperature}_seed{manual_seed}_{self.time_data}'
        os.makedirs(os.path.join(self.game_dir,self.model_dir),exist_ok=True)

        os.system('cp Double_Dueling_DQN_LLM.py '+os.path.join(self.game_dir,self.model_dir,'Double_Dueling_DQN_LLM.py'))
    ...
    def update_weights(self):
        ...
        with torch.no_grad():
            next_actions = self.model(next_states_t).max(1)[1]
            next_action_values = self.target_model(next_states_t).gather(1,next_actions.unsqueeze(-1)).squeeze(-1)
