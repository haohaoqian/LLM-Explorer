...
class Agent:
    def __init__(self, mode="training"):
        ...
        self.model_dir=f'DQN_ablation_{LLM_name}_adjust{adjust_frequency}_sample{sample_rate}_{prompt_type}_temperature{LLM_temperature}_seed{manual_seed}_{self.time_data}'
...
for episode in tqdm(range(episodes)):
    ...
    if episode != 0 and episode % adjust_frequency == 0:
        system_prompt = 'You are determining the probability distribution for exploration actions in reinforcement learning. ' + env_describe[env_name]

        step = len(action_record) // sample_rate
        action_gpt_sample = action_record[len(action_record) % sample_rate::step]
        base_prompt1 = 'In the last episode, the reward is [reward], and the action sequence extracted at intervals is ([action]). '
        ...
        base_prompt2 = f'Based on the above information, please analyze what kind of actions should be selected to better improve the task effectiveness. Please output the distribution of the {action_dim} action explorations for the next episode based on your analysis in decimal form. Your output format should be: {{'
        ...
        for _ in range(LLM_max_try):
            try:
                ...
                model=LLM_name,
                messages=[{"role": "system", "content": system_prompt},{"role": "user", "content": base_prompt1 + base_prompt2}],
                temperature=LLM_temperature)
                ans = completion.choices[0].message.content

                usage_stage=completion.usage
                output_tokens=usage_stage.completion_tokens
                prompt_tokens=usage_stage.prompt_tokens
                total_tokens=usage_stage.total_tokens
        ...
        token_input.append(prompt_tokens)
        token_output.append(output_tokens)

        np.save(os.path.join(agent.game_dir,agent.model_dir,'token_input.npy'),np.array(token_input))
        np.save(os.path.join(agent.game_dir,agent.model_dir,'token_output.npy'),np.array(token_output))

        with open(os.path.join(agent.game_dir,agent.model_dir, f'llm_text.txt'), 'a') as file:
            file.write(f'=====================Episode{episode}====================\n'+\
                       'Input:'+base_prompt1+base_prompt2+'\n-----------------------\n\n'+\
                       'Output:'+str(ans)+'\n\n\n')
