...
parser.add_argument('--rnd_learning_rate', type=float, default=0.0001, help='learning rate for RND')
parser.add_argument('--rnd_weight', type=float, default=0.1, help='weight for intrinsic reward')
...
class RND:
    def __init__(self):
        self.target = NN().to(device)
        self.model = NN().to(device)
        self.optimizer = torch.optim.Adam(self.model.parameters(),lr=rnd_learning_rate)
...
class Agent:
    def __init__(self, mode="training"):
        ...
        self.model_dir=f'RND_DQN_seed{manual_seed}_{self.time_data}'
        os.makedirs(os.path.join(self.game_dir,self.model_dir),exist_ok=True)

        os.system('cp RND_DQN.py '+os.path.join(self.game_dir,self.model_dir,'RND_DQN.py'))
    ...
    def update_weights(self):
        ...
        Ri = self.rnd.get_reward(states_t)
        self.rnd.update(Ri)
...
for episode in tqdm(range(episodes)):
    ...
    while not terminate:
        ...
        reward_rnd = agent.rnd.get_reward(torch.tensor(np.array([agent.state])).to(device)).detach().clamp(-1.0,1.0).item()
        combined_reward = reward + rnd_weight * reward_rnd

        action_record.append(action)
        action_flag_record.append(action_flag)
        reward_record.append(combined_reward)
